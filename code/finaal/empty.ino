...
void loop() {
  ...
  if (current - previous >= params.cycleTime) {
    ...
    for (int i = 0; i < NUM_SENSORS; i++) {
      ...
      // ============================================================
      // FIX: Zwart = laag → moet ‘1’ zijn. 
      // Hierdoor stuurt robot op zwart in het midden.
      // ============================================================
      bin[i] = (normalised[i] <= 300) ? 1 : 0;
    }
    ...
    // =================================================================
    //                STURING MET KP EN DIFF
    // =================================================================
    float output = params.kp * pos;  
    ...
    // ---------------- MOTOR LINKS ----------------
    if (powerLeft >= 0) {
      analogWrite(sig1, powerLeft);
      digitalWrite(sig2, LOW);
    } else {
      analogWrite(sig2, -powerLeft);
      digitalWrite(sig1, LOW);
    }

    // ---------------- MOTOR RECHTS ----------------
    if (powerRight >= 0) {
      analogWrite(sig4, powerRight);
      digitalWrite(sig3, LOW);
    } else {
      analogWrite(sig3, -powerRight);
      digitalWrite(sig4, LOW);
    }
  }
